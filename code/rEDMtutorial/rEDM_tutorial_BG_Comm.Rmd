# rEDM_tutorial_BG_Comm.Rmd

BreakoutGroup Notes from TESA Workshop Nov21-25 2016 Nanaimo, with annotations to **rEDM_tutorial.Rmd** denoted with 'BG'.

[Doesn't render in Andy's MarkdownPad2 unlike **rEDM_tutorial.Rmd** - Jackie had same issue so it's probably due to a 'BG' comment somewhere. Feel free to fix it!]
...
- George Sugihara
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette_comments
vignette: >
  %\VignetteIndexEntry{rEDM User Guide}
...
###BG notes
<<In the Breakout group we also just did the leave-out-one (by simply not putting in lib or pred; this would be required for most fisheries data that is very short in length e.g. snow crab 29 years)>>
...
<<(The BG group noted that red noise is autocorrelated noise.)>>
...
###BG note
<<First use simplex to estimate optimal E; then go to Smap and vary theta to test for nonlinear dynamics (i.e. theta>0)>>
...
###BG notes: 
<<Here we expected theta=0 rho to be around 0.95 as above in the Simplex, but then we figured out that in Smap  nn<>4 but nn=all data. As theta increases, rho improves above the rho from simplex -- so indicates non-linear behaviour assumption is better.  So using EDM is better than linear modeling.  Here if rho is zero, then there is likely simpler ways of modeling the system than EDM.>>
...
```
###BG notes
<<When doing the snow crab example, we manually entered in R1, R2 and R3 with _t, _t-1, _t-2 'lags' as with this example (i.e. our input data was 9 columns).  We finally decided from ?columns that we should have data R1, R2 and R3 (3 columns ) with those noted in columns = and target column = R1 (the one we are trying to predict).  The data illustrated above is the result of makeblock function, which took Xt, Yt, and Zt and lagged them by t-1, t-2 to make this block of data.  Make block function in rEDM takes the input data, makes the block with the correct offset years for block lnlp function to do the analysis.  Max-lag is the number of additional columns you wish for a variable and tau is the timestep used in offseting, e.g. max-lag=3 and tau=2 gives Xt, Xt-2, Xt-4.

In fact there is no need to use make block function, you could offset the data manually and have that as a data.frame that you use in block lnlp.

The block-lnlp function does multivariate EDM with either simplex or smap method.  The simplex and smap functions separately are univariate time series only, where column is time index and column two is the variable simplex or smap looks at (ignoring any other columns)

Also important, the BG noted that when using multiple variables you need
to normalize (mean of zero, stdev of 1) each variable to account for different scaling between
variables -- need to be on the same scale in order to map on the same
manifold surface, especially when the nearest neighbour of several variables is used to forecast>>
...
###BG notes
<<block-lnlp function embeds columns into the target-column.  This embedding implies that the columns are offset by the time index prior to embedding.  You can offset by either letting block-lnlp function do it with tp=some number, or you do it manually.  Here tp=0 means that you have manually offset the data already, and that you want to use the column variables topredict the same year that aligns with them....do not have EDM to offset via tp=1 or whatever.  Using a single value does not address multiple variables that have different
offsets from the target variable.

block-lnlp replaces lagged target variable used in simplex with 
external variables.  You do simplex first to get highest rho and
lowest mae combo to decide the appropriate embedding dimensions.  Then within block-lnlp embed the appropriate number of external
variables to replicate the simplex dimensions---essentially these 
external variables are replacing the target variable's time lagged series in simplex.

when dealing with a single variable and it's internal signal of an attractor, embedding means time lag; when dealing with external variables to predict that target variable's attractor, embedding means offset>>
...
###BG Notes
<<How would you actually use an EDM model to make predictions?  Once you have developed your EDM model, say in simplex or in block-lnlp, then re-do with lib=whole time series as before, but add in rows no observation in target-column, but the observed external variables.  Then it will give you a prediction for that time index>>
